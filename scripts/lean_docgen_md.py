...
def lean_type_after_decl(text: str, decl_start: int) -> str:
    ...
    if stop:
        ...
        blank = re.search(r"\n\s*\n", segment[search_from:])
        raw = (
            segment[: search_from + blank.start()].strip()
            if blank
            else segment[:400].strip()
        )
    raw = re.sub(r"\s+", " ", raw)
...
def main():
    ...
    parser.add_argument(
        "--src",
        default="legal-argument-lean/references",
        help="Directory with .lean files",
    )
    parser.add_argument(
        "--out", default="docs/references", help="Output directory for .md files"
    )
    ...
    for lf in lean_files:
        ...
        index_rows.append(
            (str(rel), parsed["namespace"], n, str(md_path.relative_to(out.parent)))
        )
