...
def to_markdown(
    ...
    for theorem, axioms in sorted(deps.items()):
        ...
        if steel:
            parts.append(
                f"**⚠️ Steelman premises ({len(steel)}):** "
                + ", ".join(f"`{a}`" for a in steel)
            )
        if legal:
            parts.append(
                f"**Legal axioms ({len(legal)}):** "
...
def main() -> None:
    ...
    parser.add_argument(
        "--input", required=True, help="axiom_audit.txt (or - for stdin)"
    )
    parser.add_argument("--out", default="docs/axiom_graph.md")
    parser.add_argument(
        "--effect",
        default="Conclusão do acórdão",
        help="Label for the Ishikawa effect (head of the fishbone)",
    )
    parser.add_argument(
        "--pretensao",
        default="Pretensão do embargante",
