...
def parse_bookmark_title(title, part_idx, default_date="0000-00-00"):
    match = re.match(
        r"^(.*?)\s*\|\s*NUM:\s*(\d+)\s*\|\s*(\d{2})/(\d{2})/(\d{4})", title
    )
    if match:
...
def _compress_with_size_fallback(
    temp_path,
    final_path,
    mode,
    title,
    ...
    is_scanned = (
        "autos digitalizados" in title.lower() or "digitalizado" in title.lower()
    )

    # Step a: If still heavy and color, try grayscale compression
    if size_per_page_kb > threshold_kb and mode not in ("bw", "gray"):
        print(
            f"Notice: Part {part_idx} is heavy after standard compression ({size_per_page_kb:.1f} KB/page)."
        )
        print(
            "Trying grayscale image compression to reduce size while preserving text layer..."
        )
        ...
        try:
            ...
            if gray_size < final_size:
                ...
                print(
                    f"Grayscale compression successful: reduced to {gray_size / 1024 / 1024:.2f} MB ({reduction:.1f}% reduction)."
                )
        ...
        except Exception as gray_err:  # noqa: BLE001
            ...
            if os.path.exists(temp_gray_path):
                for attempt in range(1, 11):
                    try:
                        ...
                        if attempt == 10:
                            print(
                                f"Warning: Could not delete {temp_gray_path}.",
                                file=sys.stderr,
                            )
                        else:
                            time.sleep(0.3)
    ...
    if size_per_page_kb > threshold_kb:
        print(
            f"Notice: Part {part_idx} remains heavy ({size_per_page_kb:.1f} KB/page, limit {threshold_kb} KB/page)."
        )
        print(
            "Applying dynamic page rasterization fallback to bypass vector/form layout bloating..."
        )
        ...
        try:
            ...
            if raster_size < final_size:
                ...
                print(
                    f"Rasterization successful: reduced to {raster_size / 1024 / 1024:.2f} MB ({reduction:.1f}% reduction)."
                )
                if os.path.exists(final_path):
                    os.remove(final_path)
                ...
                print(
                    "Rasterization did not yield a smaller file size. Keeping original compressed PDF."
                )
        except Exception as re_err:  # noqa: BLE001
            print(
                f"Warning: Rasterization fallback failed: {re_err}. Keeping original compressed PDF.",
                file=sys.stderr,
            )
        finally:
            gc.collect()
            if os.path.exists(temp_raster_path):
                for attempt in range(1, 11):
                    try:
                        ...
                        if attempt == 10:
                            print(
                                f"Warning: Could not delete {temp_raster_path}.",
                                file=sys.stderr,
                            )
                        else:
                            time.sleep(0.3)
...
def split_and_compress_toc(
    ...
    if not level1_items:
        print(
            "Warning: No level 1 bookmarks found in the PDF. Cannot split by chapters.",
            file=sys.stderr,
        )
        doc.close()
        sys.exit(1)
    ...
    for part in parts:
        ...
        if nup > 1 and also_plain:
            plain_part_filename = (
                part_filename[:-4] + "_plain.pdf"
                if part_filename.endswith(".pdf")
                else part_filename + "_plain.pdf"
            )
            ...
            try:
                plain_mode = mode
                if (
                    "autos digitalizados" in title.lower()
                    or "digitalizado" in title.lower()
                ):
                    plain_mode = "bw"
            ...
            except Exception as plain_err:  # noqa: BLE001
                print(
                    f"Warning: plain compression of part {part_idx} failed: {plain_err}. Keeping uncompressed version.",
                    file=sys.stderr,
                )
        ...
        try:
            ...
            if (
                ...
                print(
                    "Forcing B&W mode ('bw') for scanned document to achieve maximum compression."
                )
                part_mode = "bw"
            ...
            print(
                f"Final part size: {final_size / 1024 / 1024:.2f} MB (Overall Reduction: {(1 - final_size / temp_size) * 100:.1f}%)"
            )
            ...
            if os.path.exists(temp_part_path):
                for attempt in range(1, 11):
                    try:
                        ...
                        if attempt == 10:
                            print(
                                f"Warning: Could not delete temporary file {temp_part_path}. It remains locked.",
                                file=sys.stderr,
                            )
                        else:
                            time.sleep(0.3)
    ...
    print(
        f"Overall Size Reduction: {(1 - total_compressed_size / orig_total_size) * 100:.1f}%"
    )
    ...
    for path, size, p_count, _ in sorted_parts[:20]:
        print(
            f" - {os.path.basename(path)}: {size / 1024 / 1024:.2f} MB ({p_count} pages)"
        )
    print("=========================================")
    ...
    plain_merged_path = os.path.join(
        input_dir, f"{name_without_ext}{plain_suffix}{ext}"
    )
    ...
    if nup > 1 and also_plain and parts_info_plain:
        ...
        print(
            f"Plain Merged File Size: {plain_merged_size / 1024 / 1024:.2f} MB (Reduction: {(1 - plain_merged_size / orig_total_size) * 100:.1f}%)"
        )
        ...
        print(
            f"Merged File Size: {plain_merged_size / 1024 / 1024:.2f} MB (Reduction: {(1 - plain_merged_size / orig_total_size) * 100:.1f}%)"
        )
    ...
    print(
        f"Merged File Size: {merged_size / 1024 / 1024:.2f} MB (Reduction: {(1 - merged_size / orig_total_size) * 100:.1f}%)"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Split PDF by bookmarks/TOC and compress each part."
    )
    parser.add_argument("--input", required=True, help="Path to input PDF file")
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory to save the split & compressed PDFs",
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--quality", type=int, default=50, help="JPEG quality (default: 50)"
    )
    parser.add_argument(
        "--threshold-kb",
        type=int,
        default=150,
        help="Threshold in KB per page to trigger rasterization fallback (default: 150)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="N-up layout (e.g. 2, 4, 8 pages per page). Default: 1. When nup>1, a plain _compressed version is also always produced.",
    )
    parser.add_argument(
        "--no-plain",
        action="store_true",
        default=False,
        help="When nup>1, skip generating the plain _compressed (nup=1) version.",
    )
