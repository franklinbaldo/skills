...
def portal_url(src):
    p = {
        "id": src.get("id_processo_documento"),
...
def orgao(src):
    return src.get("ds_orgao_julgador_colegiado") or src.get("ds_orgao_julgador") or "—"
...
def trecho_em_torno(txt, termo, raio=200):
    ...
    pos = -1
    if termo:
        m = re.search(re.escape(termo), txt, re.IGNORECASE)
        if m:
            pos = m.start()
    if pos < 0:
            ...
            out.append(
                {
                    ...
                    "sistema_origem": src.get("sistema_origem"),
                    "url": portal_url(src),
                    "trecho": trecho_em_torno(
                        txt, args.trecho_perto or args.termo, 200
                    ),
                }
            )
        print(
            json.dumps(
                {"total_no_acervo": total, "retornados": len(out), "resultados": out},
                ensure_ascii=False,
                indent=2,
            )
        )
...
def cmd_processo(args):
    ...
    if args.json:
        ...
        print(
            json.dumps(
                {"processo": cnj(nr), "documentos": len(out), "itens": out},
                ensure_ascii=False,
                indent=2,
            )
        )
...
def cmd_texto(args):
    ...
    print(
        f"# {src.get('ds_classe_judicial')} | {src.get('dtjulgamento_str')} | "
...
def build_parser():
    ...
    b.add_argument(
        "--tamanho", type=int, default=20, help="numero de resultados (padrao 20)"
    )
    b.add_argument(
        "--repetidos", action="store_true", help="nao deduplicar por numero de processo"
    )
    b.add_argument(
        "--trecho-perto",
        metavar="TERMO",
    ...
    t.add_argument(
        "id", help="id_processo_documento (campo 'id_documento' dos resultados)"
    )
    t.add_argument(
        "--max", type=int, default=0, help="trunca em N caracteres (0 = sem limite)"
    )
    ...
    f.add_argument(
        "--limite", type=int, default=15, help="itens por faceta (padrao 15)"
    )
