...
class UnifiedProjectionTests(unittest.TestCase):
    def test_projects_static_layers_and_rfc0006_contracts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(counts["declared_types"], 6)
            ...
            self.assertFalse((output / "benchmark-rnd").exists())

            manifest = (output / "projection.md").read_text(encoding="utf-8")
            self.assertNotIn("routing_repetitions", manifest)
            self.assertNotIn("planned_routing_run_count", manifest)
            self.assertNotIn("challenge_case_count", manifest)
