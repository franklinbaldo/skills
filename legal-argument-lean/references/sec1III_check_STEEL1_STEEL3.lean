/-
  ...
  *paradigmaticamente* fundamentação universalizável.
  ...
  inertes para Alice. A fundamentação implícita do acórdão
...
/-- **V1b — versão restringida com fatos incontroversos.**
    ...
    STF admite em qualificação jurídica (RE 210.917). -/
def V1b : Prop :=
...
/-- **V1d — versão tautológica/defensável.** Súmula 279 incide
    ...
    Alice (acórdão recorrido assenta tudo). Logo: tautológica
    e inerte. -/
def V1d : Prop :=
...
/-- Caso paradigma do RE 210.917, Pertence, Tribunal Pleno —
...
/-- **V1a viola §1º III.** Aplicado ao caso paradigma do
    RE 210.917 (próprio leading case da distinção
...
/-- **V1b viola §1º III.** Mesmo argumento, adicionando o
    ...
    paradigmaticamente no RE 210.917. -/
...
/-- **V1c viola §1º III.** Versão deferente restringe à
    matéria de RPPS, mas o RE 210.917 (e numerosos outros)
...
/-- **V1d passa no §1º III check.** A precondition
    ...
    para Alice (acórdão recorrido assenta os fatos). -/
...
/-- **V3b — versão correta/tautológica.** Precedente sobre
    ...
    §1º III check. Mas no caso de Alice, os precedentes
