...
def clean_text(text):
    ...
    text = re.sub(
        r"Poder Judiciário\s*Superior Tribunal de Justiça\s*Diário de Justiça Eletrônico Nacional de \d\d/\d\d/\d\d\d\d\s*Certidão de publicação \d+",
        "",
        text,
    )
...
def split_and_save_documents(pages_data, outdir, base_pdf_name):
    ...
    def save_current_doc():
        ...
        doc_counter += 1
        clean_title = re.sub(
            r"[^a-zA-Z0-9_]", "_", current_doc_title.strip().replace(" ", "_")
        ).lower()
        # Truncate title
...
def generate_index(outdir, base_pdf_name, index_entries):
    ...
    with open(index_path, "w", encoding="utf-8") as f:
        ...
        f.write("|---|-----------|---------|-----------------|\n")
        f.writelines(
            f"| {seq} | **{title}** | [{filename}](./{filename}) | {snippet} |\n"
...
def convert_with_markitdown(
    pdf_path, keep_data_uris=False, docintel_endpoint=None, cu_endpoint=None
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert court process PDFs into clean structured Markdown folders."
    )
    parser.add_argument("--input", required=True, help="Path to the input PDF file.")
    parser.add_argument(
        "--outdir", required=True, help="Directory to save the markdown files."
    )
    parser.add_argument(
        "--keep-data-uris", action="store_true", help="Keep data URIs in output."
    )
    parser.add_argument(
        "--docintel-endpoint", help="Azure Document Intelligence Endpoint URL."
    )
    parser.add_argument(
        "--cu-endpoint", help="Azure Content Understanding Endpoint URL."
    )
